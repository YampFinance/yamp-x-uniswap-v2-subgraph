...
import { ERC20 } from '../types/YampFactory/ERC20'
import { ERC20SymbolBytes } from '../types/YampFactory/ERC20SymbolBytes'
import { ERC20NameBytes } from '../types/YampFactory/ERC20NameBytes'
import { YAMP_FACTORY_ADDRESS, UNISWAP_FACTORY_ADDRESS } from './constants'
import { YampFactory, Borrowable, Collateral, LendingPool, Token, Pair, Distributor, User, CollateralPosition, SupplyPosition, BorrowPosition } from "../types/schema"
import { UniswapFactory as UniswapFactoryContract } from '../types/YampFactory/UniswapFactory'
import { Pair as PairContract } from '../types/YampFactory/Pair'
import { FarmingPool as FarmingPoolContract } from '../types/YampFactory/FarmingPool'
import { Distributor as DistributorContract } from '../types/YampFactory/Distributor'
...
export function updateLendingPoolUSD(pairAddress: String): void {
  ...
  let yampFactory = YampFactory.load(YAMP_FACTORY_ADDRESS)
  if (yampFactory === null) return
  yampFactory.totalBalanceUSD = yampFactory.totalBalanceUSD.plus(lendingPool.totalBalanceUSD).minus(prevTotalBalanceUSD)
  yampFactory.totalSupplyUSD = yampFactory.totalSupplyUSD.plus(lendingPool.totalSupplyUSD).minus(prevTotalSupplyUSD)
  yampFactory.totalBorrowsUSD = yampFactory.totalBorrowsUSD.plus(lendingPool.totalBorrowsUSD).minus(prevTotalBorrowsUSD)
  yampFactory.save()
}