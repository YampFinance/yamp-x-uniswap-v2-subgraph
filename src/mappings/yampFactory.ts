...
} from "../types/YampFactory/YampFactory"
import { YAMP_FACTORY_ADDRESS, UNISWAP_FACTORY_ADDRESS } from './constants'
import { YampFactory, Pair, Bundle, Token, LendingPool, Collateral, Borrowable } from "../types/schema"
...
export function handleLendingPoolInitialized(event: LendingPoolInitialized): void {
  ...
  if (yampFactory === null) {
	yampFactory = new YampFactory(YAMP_FACTORY_ADDRESS)
    yampFactory.totalBalanceUSD = ZERO_BD
    yampFactory.totalSupplyUSD = ZERO_BD
    yampFactory.totalBorrowsUSD = ZERO_BD
